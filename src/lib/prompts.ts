...
export function buildStorefrontPrompt(shopName: string): string {
  const name = shopName.trim();
  return `输入的店铺名：${name}。请参考输入的店铺名和上传生成的头像图，为这个店铺设计生成一张醒目且极具冲击力的16:9横版店招宣传图，采用极具戏剧性的商业食品摄影风格，商业海报式的构图，以及高端诱人的快餐广告美学。图中不要加入促销价格、满减信息、二维码、地址、电话、联系方式或其他无关营销元素。`;
}

/**
 * 海报 prompt：参考头像图，生成横版广告海报。
 */
export function buildPosterPrompt(shopName: string): string {
  const name = shopName.trim();
  return `输入的店铺名：${name}。请参考已生成的头像宣传图，为这个店铺设计生成一张醒目且极具冲击力的21:9横版广告电商海报图，采用极具戏剧性的商业食品摄影风格，商业海报式的构图，以及高端诱人的快餐广告美学。图中不要加入促销价格、满减信息、二维码、地址、电话、联系方式或其他无关营销元素。`;
}
