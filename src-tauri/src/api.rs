...
pub async fn generate_image(req: GenerateRequest) -> Result<String, String> {
    ...
    if req.api_line == ImageApiLine::Line7 && !req.product_images.is_empty() {
        let edit_api_url = provider
            .edit_api_url
            .ok_or_else(|| "线路7编辑接口未配置".to_string())?;
        let image = generate_novaeworld_edit_image(
            &client,
            edit_api_url,
            &api_key,
            provider.model,
        ...
        .await?;
        return download_image_if_url(&client, image, "下载线路7编辑远端图片失败").await;
    }
