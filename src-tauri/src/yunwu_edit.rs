...
pub async fn generate_yunwu_edit_image(
    ...
    for (index, image_ref) in image_refs.iter().enumerate() {
        ...
        let part = Part::bytes(file.bytes)
            ...
            .mime_str(&file.mime)
            .map_err(|error| format!("构建线路2参考图表单失败：{error}"))?;
    ...
    let response = client
        ...
        .send()
        .await
        .map_err(|error| format!("调用线路2编辑接口失败：{}", format_reqwest_error(&error)))?;

    let status = response.status();
    eprintln!("[image-2:line2-edit] response_status={status}");
    let body_text = response
        .text()
        .await
        .map_err(|error| format!("读取线路2编辑响应失败：{error}"))?;
    eprintln!(
        "[image-2:line2-edit] response_preview={}",
...
async fn download_reference_image(
    ...
    let bytes = response
        .error_for_status()
        .map_err(|error| format!("下载线路2第 {} 张参考图失败：{error}", index + 1))?
        .bytes()
        .await
        .map_err(|error| format!("读取线路2第 {} 张参考图失败：{error}", index + 1))?;
...
fn decode_reference_image(image_ref: &str, index: usize) -> Result<ReferenceImageFile, String> {
    ...
    let bytes = STANDARD
        .decode(b64.trim())
        .map_err(|error| format!("解析线路2第 {} 张 base64 参考图失败：{error}", index + 1))?;
    Ok(ReferenceImageFile {
        bytes,
        mime: mime.to_string(),
        file_name: format!("reference-{}.{}", index + 1, extension_from_mime(mime)),
