...
ok(queue.includes("self.line_health.snapshot()"), "服务端队列应在分配线路时读取最新线路健康状态");
ok(/api_line:\s*line/.test(gateway), "网关调用生图接口前应把 auto 替换成实际线路");
ok(/line_health\s*\.\s*record\(\s*line\.as_str\(\)/.test(gateway), "网关应按实际线路记录健康样本");

for (const line of ["line2", "line3", "line4", "line5", "line6", "line7"]) {
  ok(limiter.includes(`"${line}"`), `自动分配候选应包含 ${line}`);
}
ok(!limiter.includes('"line1"'), "线路1移除后不应进入自动分配候选");
ok(!limiter.includes('"line8"'), "线路8移除后不应进入自动分配候选");
ok(
  limiter.includes('AUTO_GENERATION_LINES: [&str; 6] = ["line2", "line3", "line4", "line5", "line6", "line7"]'),
  "自动分配应保留 line2-line7 顺序"
);
ok(limiter.includes("supports_generation_size"), "自动分配应按请求尺寸筛掉不兼容线路");
ok(
  limiter.includes('"16:9" | "auto" | "1792x1024" => Cow::Borrowed("16:9")'),
  "线路2店招应保持 16:9，不应映射成海报尺寸"
);
ok(
  limiter.includes('"21:9" | "1792x768" => Cow::Borrowed("21:9")'),
  "线路2海报应保持 21:9，并兼容旧的 1792x768"
);
...
ok(!topbar.includes("自动分配线路"), "顶部状态不应再显示自动分配线路文案块");
