...
for (const tool of tools) {
  // 每个 hook 应在 useGenerationWorkspace 实例化 10 个 slot
  const matches = workspace.match(new RegExp(tool.name, "g")) ?? [];
  ok(matches.length >= 10, `${tool.name} 应被调用 10 次（每店一次），实际 ${matches.length}`);
...
console.log("multi-store ten-store contract: OK");
