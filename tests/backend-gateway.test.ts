...
equal(tauriSource.includes("BACKEND_GATEWAY_REQUEST_TIMEOUT_MS = 600_000"), true);
...
equal(gatewaySource.includes("DOUYIN_COOKIE_PATH"), true);
equal(gatewaySource.includes("DOUYIN_YTDLP_PYTHON"), true);
equal(gatewaySource.includes("GenerateImageResponse"), true);
equal(gatewaySource.includes("generation_line"), true);
equal(gatewaySource.includes("/api/upload-image-to-oss"), true);
equal(gatewaySource.includes("/api/global-generation-total"), true);
equal(gatewaySource.includes("global_generation_total"), true);
equal(gatewaySource.includes("generation_totals?select=total_count"), true);
equal(gatewaySource.includes("/api/admin/account-generation-summary"), true);
